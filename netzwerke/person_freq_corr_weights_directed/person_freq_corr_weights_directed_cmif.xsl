        ...
        <xsl:for-each-group select="//tei:correspDesc[tei:correspContext/tei:ref[@type='belongsToCorrespondence']]"
            ...
            <xsl:if test="$korr-id != 'correspondence_null'">
                ...
                <xsl:if test="$mentioned-persons/person">
                    ...
                    <xsl:result-document href="../../netzwerke/person_freq_corr_weights_directed/person_freq_corr_weights_directed_{$korr-id}_alle.csv">
                    ...
                    <!-- Create _top30.csv if more than 30 persons -->
                    <xsl:if test="$total-persons &gt; 30">
                        <xsl:result-document href="../../netzwerke/person_freq_corr_weights_directed/person_freq_corr_weights_directed_{$korr-id}_top30.csv">
                            <xsl:text>Source,Target,Weight,Type&#10;</xsl:text>
                            <xsl:for-each select="$mentioned-persons/person">
                                <xsl:sort select="@count" data-type="number" order="descending"/>
                    ...
                    <!-- Create _top100.csv if more than 100 persons -->
                    <xsl:if test="$total-persons &gt; 100">
                        <xsl:result-document href="../../netzwerke/person_freq_corr_weights_directed/person_freq_corr_weights_directed_{$korr-id}_top100.csv">
                            <xsl:text>Source,Target,Weight,Type&#10;</xsl:text>
                            <xsl:for-each select="$mentioned-persons/person">
                                <xsl:sort select="@count" data-type="number" order="descending"/>
