            ...
            <xsl:if test="$korr-id != 'correspondence_null'">
                ...
                <xsl:if test="$mentioned-works/work">
                    ...
                    <xsl:variable name="suffix">
                        <xsl:choose>
                            <xsl:when test="$total-works &gt; 100">_top100</xsl:when>
                            <xsl:when test="$total-works &gt; 30">_top30</xsl:when>
                            <xsl:otherwise>_alle</xsl:otherwise>
                        </xsl:choose>
                    </xsl:variable>

                    <!-- Create CSV file -->
                    <xsl:result-document href="../../netzwerke/work_freq_corr_weights_directed/work_freq_corr_weights_directed_{$korr-id}{$suffix}.csv">
                        <xsl:text>Source,Target,Weight,Type&#10;</xsl:text>

                        <!-- Sort by count descending and take top N -->
                        <xsl:variable name="limit">
                            <xsl:choose>
                                <xsl:when test="$total-works &gt; 100">100</xsl:when>
                                <xsl:when test="$total-works &gt; 30">30</xsl:when>
                                <xsl:otherwise><xsl:value-of select="$total-works"/></xsl:otherwise>
                            </xsl:choose>
                        </xsl:variable>
