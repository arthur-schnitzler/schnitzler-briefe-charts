    ...
    <xsl:template match="/">
        ...
        <xsl:for-each-group select="//tei:correspDesc[tei:correspContext/tei:ref[@type='belongsToCorrespondence']]"
            ...
            <xsl:if test="$korr-id != 'correspondence_null'">
                ...
                <!-- Collect all mentioned institutions in this correspondence -->
                <xsl:variable name="mentioned-orgs">
                    <xsl:for-each-group select="current-group()//tei:note/tei:ref[@type='https://lod.academy/cmif/vocab/terms#mentionsOrg']"
                        group-by="@target">
                        <xsl:variable name="count" select="count(current-group())"/>
                        <xsl:variable name="org-name" select="normalize-space(current-group()[1])"/>
                        <org id="{current-grouping-key()}" count="{$count}">
                            <xsl:value-of select="$org-name"/>
                        </org>
                    </xsl:for-each-group>
                </xsl:variable>

                <!-- Only create CSV if there are institutions -->
