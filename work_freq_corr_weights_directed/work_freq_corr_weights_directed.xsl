...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <!-- this template creates a csv file for network analysis 
         containing all correspondences with the 30 most frequently mentioned works in each correspondence with weights and directions -->
    ...
    <xsl:template match="/">
        ...
        <xsl:for-each select="//tei:personGrp[@xml:id != 'correspondence_null']">
            ...
            <xsl:if test="$works/*:work">
                ...
                <xsl:result-document href="work_freq_corr_weights_directed_{$korr-id}.csv"
                    method="text">
